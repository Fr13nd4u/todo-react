...
function App() {
  ...
  useEffect(() => { // альтернатива componentDidMount
  ...
  }, []); // если [] то вызываеться 1 раз, если [lists] то будет вызываться еще если lists обновиться

  const onAddList = obj => {
    const newLists = [...lists, obj];  // спреад оператор (конкатенация старого обьета + новый newLists)
    setLists(newLists);
  };
